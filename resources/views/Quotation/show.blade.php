                    ...
                    <div class="overflow-x-auto">
                        <x-auth-validation-errors class="mb-4" :errors="$errors" />
                                                ...
                                                <td>
                                                    @if ($service_request->customer_approval == null && Auth::user()->hasRole('staff'))
                                                        <form action="{{ route('quotation.delete', ['quotation'=>$quotation]) }}" method="POST">
                                                            @method('DELETE')
                                                            @csrf                                       
                                                            <input class="btn btn-sm bg-red-500 hover:bg-blue-500 text-white font-bold py-2 px-4 rounded" type="submit" value="Delete">
                                                        </form> 
                                                    
                                                    @elseif (Auth::user()->hasRole('staff') && $service_request->customer_approval == null)
                                                    ...
                                                    @elseif (Auth::user()->hasRole('customer') && $service_request->customer_approval == null)
                                                        <a class="btn bg-green-500 hover:bg-blue-500 text-white font-bold py-2 px-4 rounded" href="{{ route('service_request.customer_approve', ['service_request'=>$service_request]) }}">Accept</a>
                                                        <a class="btn bg-red-500 hover:bg-blue-500 text-white font-bold py-2 px-4 rounded" href="{{ route('service_request.customer_not_approve', ['service_request'=>$service_request]) }}">Reject</a>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach
                            ...
                            <div class="inline-flex">
                                <button class="bg-gray-400 hover:bg-blue-500 text-black font-bold py-2 px-4 rounded" onclick="location.href='{{ url('/quotation') }}'">
                                        Back
                                </button>                         
                            </div> 
                    </div>                      
